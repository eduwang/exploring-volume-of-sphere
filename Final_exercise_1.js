...
function init() {
    ...
    camera.position.set(6.13, 5.14, 7.12);
    camera.quaternion.set(-0.3, -0.01, 0, 0.97);
    ...
    document.body.appendChild(renderer.domElement);
        ...
        document.getElementById('rotateScene').addEventListener('click', rotateScene);
    ...
    // controls.minPolarAngle = Math.PI / 2; // Lock vertical rotation at 90 degrees
    // controls.maxPolarAngle = Math.PI / 2; // Lock vertical rotation at 90 degrees
...
function createAndVisualizeTube(radius, height, segments, index, color) {
    ...
    tubeBody.position.set(radius * 6, offset, 0); // Position to the right of the hemispheres
...
function createAndVisualizeBottomPlane(size) {
    ...
    bottomPlaneBody.position.set(size *3 -3, -2-0.1, 0); // Slightly below the bottom of the lowest tube
    ...
    mesh.position.set(size * 3-3, -2, 0);
...
function createAndVisualizeTopPlane(size) {
    ...
    topPlaneBody.position.set(size * 3 -3, 2, 0); // Adjust position based on new tube position
...
function addBallToCylinder() {
    ...
    const ballBody = new CANNON.Body({ mass: 1 });
    ballBody.addShape(ballShape);
    const xPosition = 12+ (Math.random() - 0.5) * 2; // Random value between -1 and 1
    const yPosition = (Math.random() - 0.5) * 0.3 + 0.8; // Random value between -1 and 1
...
function saveCameraPosition() {
    ...
    // Log the position and rotation for debugging purposes
    console.log('Saved Camera Position:', cameraPosition);
    console.log('Saved Camera Quaternion:', cameraQuaternion);

    // You can store these values and use them to set the camera position later
    // For example, save them in localStorage, or use them in another function
}
